 ...
 * 
 * <p>
 * <b>WARNING</b>: This API is a new and experimental and may suddenly change.
 * </p>
 */
final public class UnicodeUtil {
	...
	/**
	 * Encode characters from a char[] source, starting at offset and stopping
	 * when the character 0xffff is seen. Returns the number of bytes written to
	 * bytesOut.
	 */
	 ...
	 * chars. Returns the number of bytes written to bytesOut.
	 */
	public static void UTF16toUTF8(final char[] source, final int offset,
	...
	public static void UTF8toUTF16(final byte[] utf8, final int offset,
		...
		if (result.offsets.length <= end) {
			...
			System.arraycopy(result.offsets, 0, newOffsets, 0,
					result.offsets.length);
	 ...
	 * (UnsupportedEncodingException uee) { return false; } }
	 * 
	 * public static final boolean validUTF16String(String s) { final int size =
	 * s.length(); for(int i=0;i<size;i++) { char ch = s.charAt(i); if (ch >=
	 * UNI_SUR_HIGH_START && ch <= UNI_SUR_HIGH_END) { if (i < size-1) { i++;
	 * char nextCH = s.charAt(i); if (nextCH >= UNI_SUR_LOW_START && nextCH <=
	 * UNI_SUR_LOW_END) { // Valid surrogate pair } else // Unmatched hight
	 * surrogate return false; } else // Unmatched hight surrogate return false; }
	 * else if (ch >= UNI_SUR_LOW_START && ch <= UNI_SUR_LOW_END) // Unmatched
	 * low surrogate return false; }
	 * 
	 * return true; }
	 * 
	 * public static final boolean validUTF16String(char[] s, int size) {
	 * for(int i=0;i<size;i++) { char ch = s[i]; if (ch >= UNI_SUR_HIGH_START &&
	 * ch <= UNI_SUR_HIGH_END) { if (i < size-1) { i++; char nextCH = s[i]; if
	 * (nextCH >= UNI_SUR_LOW_START && nextCH <= UNI_SUR_LOW_END) { // Valid
	 * surrogate pair } else return false; } else return false; } else if (ch >=
	 * UNI_SUR_LOW_START && ch <= UNI_SUR_LOW_END) // Unmatched low surrogate
	 * return false; }
	 * 
	 * return true; }
	 */
