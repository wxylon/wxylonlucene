 ...
 * instead always throw UnsupportedOperationException. Subclasses are strongly
 * encouraged to override these methods, but in many cases may not need to.
 * </p>
 * 
 * <p>
 * 
 * <b>NOTE</b>: as of 2.4, it's possible to open a read-only IndexReader using
 * one of the static open methods that accepts the boolean readOnly parameter.
 * Such a reader has better concurrency as it's not necessary to synchronize on
 * the isDeleted method. Currently the default for readOnly is false, meaning if
 * not specified you will get a read/write IndexReader. But in 3.0 this default
 * will change to true, meaning you must explicitly specify false if you want to
...
public abstract class IndexReader {
	...
	/**
	 * Expert: increments the refCount of this IndexReader instance. RefCounts
	 * are used to determine when a reader can be closed safely, i.e. as soon as
	 * there are no more references. Be sure to always call a corresponding
	 ...
	 * refCount drops to 0, then pending changes (if any) are committed to the
	...
	/**
	 * Returns a read/write IndexReader reading the index in an FSDirectory in
	 * the named path. <b>NOTE</b>: starting in 3.0 this will return a readOnly
	 * IndexReader.
	 * 
	...
	/**
	 * Returns a read/write IndexReader reading the index in the given
	 * Directory. <b>NOTE</b>: starting in 3.0 this will return a readOnly
	 * IndexReader.
	 * 
	 * @param directory
	 *            the index directory
	...
	/**
	 * Returns a read/write or read only IndexReader reading the index in the
	 * given Directory.
	 * 
	 * @param directory
	 *            the index directory
	 ...
	 * {@link IndexCommit}. <b>NOTE</b>: starting in 3.0 this will return a
	...
	/**
	 * Expert: returns a read/write IndexReader reading the index in the given
	 * Directory, with a custom {@link IndexDeletionPolicy}. <b>NOTE</b>:
	 * starting in 3.0 this will return a readOnly IndexReader.
	 * 
	 * @param directory
	 *            the index directory
	...
	/**
	 * Expert: returns a read/write or read only IndexReader reading the index
	 * in the given Directory, with a custom {@link IndexDeletionPolicy}.
	 * <b>NOTE</b>: starting in 3.0 this will return a readOnly IndexReader.
	 * 
	 * @param directory
	 *            the index directory
	 * @param deletionPolicy
	 ...
	 * {@link IndexDeletionPolicy}. <b>NOTE</b>: starting in 3.0 this will
	 * return a readOnly IndexReader.
	 * 
	 * @param commit
	 *            the specific {@link IndexCommit} to open; see
	 *            {@link IndexReader#listCommits} to list all commits in a
	 *            directory
	 * @param deletionPolicy
	 *            a custom deletion policy (only used if you use this reader to
	 *            perform deletes or to set norms); see {@link IndexWriter} for
	...
	/**
	 * Expert: returns a read/write or read only IndexReader reading the index
	 * in the given Directory, using a specific commit and with a custom
	...
	private static IndexReader open(final Directory directory,
			...
			final boolean readOnly) throws CorruptIndexException, IOException {
	 ...
	 * term greater than the supplied term. The enumeration is ordered by
	...
	/**
	 * Expert: return the IndexCommit that this reader has opened. This method
	 * is only implemented by those readers that correspond to a Directory with
	 * its own segments_N file.
	 * 
	 * <p>
	...
	/**
	 * Returns all commit points that exist in the Directory. Normally, because
	 * the default is {@link KeepOnlyLastCommitDeletionPolicy}, there would be
	 * only one commit point. But if you're using a custom {@link
	 * IndexDeletionPolicy} then there could be many commits. Once you have a
	 * given commit, you can open a reader on it by calling
	 * {@link IndexReader#open(IndexCommit)} There must be at least one commit
	 * in the Directory, else this method throws {@link java.io.IOException}.
	 * Note that if a commit is in progress while this method is running, that
	 * commit may or may not be returned array.
	 */
