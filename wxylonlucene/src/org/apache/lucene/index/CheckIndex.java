...
/**
 * Basic tool and API to check the health of an index and write a new segments
 * file that removes reference to problematic segments.
 * <p>
 * As this tool checks every byte in the index, on a large index it can take
 * quite a long time to run.
 * <p>
 * <b>WARNING</b>: this tool and API is new and experimental and is subject to
 * suddenly change in the next release. Please make a complete backup of your
...
public class CheckIndex {
	 ...
	 * @deprecated Use {@link #setInfoStream} per instance, instead.
	 */
	...
	/**
	 * Returned from {@link #checkIndex()} detailing the health and status of
	 * the index.
	 * 
	 * <p>
	 * <b>WARNING</b>: this API is new and experimental and is subject to
	 * suddenly change in the next release.
	 */
	...
	public static class Status {
		...
		/**
		 * True if we were unable to read the version number from segments_N
		 * file.
		 */
		...
		/**
		 * Empty unless you passed specific segments list to check as optional
		 * 3rd argument.
		 * 
		 ...
		 * (this is used with the {@link CheckIndex#fix} method to repair the
		 * index.
		 */
		 ...
		 * #checkIndex(List)}) was called with non-null argument).
		 */
		...
		public static class SegmentInfoStatus {
			 ...
			 * (stored fields and term vectors) with other segments. This is -1
			 * if it does not share.
			 */
			public int docStoreOffset = -1;

			/**
			 * String of the shared doc store segment, or null if this segment
			 * does not share the doc store files.
			 */
			...
			/**
			 * True if at least one of the fields in this segment does not
			 * omitTf.
			 * 
	 ...
	 * 
	 * @deprecated Please instantiate a CheckIndex and then use
	 *             {@link #checkIndex()} instead
	 */
	 ...
	 * 
	 * <p>
	 * As this method checks every byte in the index, on a large index it can
	 * take quite a long time to run.
	 * 
	 * <p>
	 * <b>WARNING</b>: make sure you only call this when the index is not
	 * opened by any writer.
	 */
	 ...
	 * 
	 * <p>
	 * As this method checks every byte in the specified segments, on a large
	...
	public Status checkIndex(List onlySegments) throws IOException {
		...
		for (int i = 0; i < numSegments; i++) {
			...
			try {
				...
				msg("OK [" + termCount + " terms; " + totFreq
				...
				msg("OK [" + totFields + " total field count; avg "
				...
				msg("OK [" + totVectors + " total vector count; avg "
	 ...
	 * {@link #checkIndex}. Note that this does not remove any of the
	 * unreferenced files after it's done; you must separately open an
	 * {@link IndexWriter}, which deletes unreferenced files when it's created.
	 * 
	 * <p>
	 * <b>WARNING</b>: this writes a new segments file into the index,
	 * effectively removing all documents in broken segments from the index. BE
	 * CAREFUL.
	 * 
	 * <p>
	 * <b>WARNING</b>: Make sure you only call this when the index is not
	 ...
	 * 
	 * <ul>
	 * <li><code>-fix</code>: actually write a new segments_N file, removing
	 * any problematic segments
	 * 
	 * <li><code>-segment X</code>: only check the specified segment(s).
	 * This can be specified multiple times, to check more than one segment, eg
	 * <code>-segment _2
	 ...
	 * 
	 * <p>
	 * <b>WARNING</b>: <code>-fix</code> should only be used on an emergency
	 * basis as it will cause documents (perhaps many) to be permanently removed
	 * from the index. Always make a backup copy of your index before running
	 * this! Do not run this tool on an index that is actively being written to.
	 * You have been warned!
	 * 
	 * <p>
	 * Run without -fix, this tool will open the index, report version
	 * information and report any exceptions it hits and what action it would
	 * take if -fix were specified. With -fix, this tool will remove any
	 * segments that have issues and write a new segments_N file. This means all
	 * documents contained in the affected segments will be removed.
	 * 
	 * <p>
