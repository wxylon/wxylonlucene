...
abstract class DirectoryIndexReader extends IndexReader {
	...
	static DirectoryIndexReader open(final Directory directory,
		...
		SegmentInfos.FindSegmentsFile finder = new SegmentInfos.FindSegmentsFile(
				directory) {
			...
			protected Object doBody(String segmentFileName)
				...
				if (infos.size() == 1) { // index is optimized
					reader = SegmentReader.get(readOnly, infos, infos.info(0),
							closeDirectory);
				} else if (readOnly) {
					reader = new ReadOnlyMultiSegmentReader(directory, infos,
							closeDirectory);
				} else {
		...
		if (commit == null)
			...
			return (DirectoryIndexReader) finder.doBody(commit
					.getSegmentsFileName());
		}
	}
	...
	public final synchronized IndexReader reopen()
		...
		if (this.hasChanges || this.isCurrent()) {
			...
			// reopen
			// OR: the index in the directory hasn't changed - nothing to do
			// here
			return this;
		}
