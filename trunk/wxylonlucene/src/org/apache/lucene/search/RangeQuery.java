...
public class RangeQuery extends Query {
	 ...
	 * @param inclusive
	 *            If true, both <code>lowerTerm</code> and
	 *            <code>upperTerm</code> will themselves be included in the
	 *            range.
	 */
	 ...
	 * index terms are within the given range, rather than using the Unicode
	 * code point order in which index terms are stored.
	 * <p>
	 * <strong>WARNING:</strong> Using this constructor and supplying a
	 * non-null value in the <code>collator</code> parameter will cause every
	 * single index Term in the Field referenced by lowerTerm and/or upperTerm
	 * to be examined. Depending on the number of index Terms in this Field, the
	 ...
	 *            membership in the range bounded by <code>lowerTerm</code>
	 *            and <code>upperTerm</code>.
	 */
	public RangeQuery(Term lowerTerm, Term upperTerm, boolean inclusive,
			Collator collator) {
			...
			try {
				do {
					...
					if (term != null && term.field() == testField) { // interned
																		// comparison
						if ((lowerTermText == null || (inclusive ? collator
								.compare(term.text(), lowerTermText) >= 0
								: collator.compare(term.text(), lowerTermText) > 0))
								&& (upperTermText == null || (inclusive ? collator
										.compare(term.text(), upperTermText) <= 0
										: collator.compare(term.text(),
												upperTermText) < 0))) {
