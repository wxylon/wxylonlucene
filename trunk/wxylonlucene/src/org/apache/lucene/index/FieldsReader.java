...
final class FieldsReader {
	...
	FieldsReader(Directory d, String segment, FieldInfos fn,
		...
		try {
			...
			cloneableFieldsStream = d.openInput(segment + "."
					+ IndexFileNames.FIELDS_EXTENSION, readBufferSize);
			indexStream = d.openInput(segment + "."
			...
			if (format > FieldsWriter.FORMAT_CURRENT)
				throw new CorruptIndexException("Incompatible format version: "
						+ format + " expected " + FieldsWriter.FORMAT_CURRENT
						+ " or lower");
					...
					try {
						...
						if (isCompressed) {
							...
							if (format >= FieldsWriter.FORMAT_VERSION_UTF8_LENGTH_IN_BYTES) {
								...
								// read in chars b/c we already know the length
								// we need to read
		...
		public byte[] getBinaryValue(byte[] result) {
			...
			if (isBinary) {
				if (fieldsData == null) {
					...
					// Throw this IOException since IndexReader.document does so
					// anyway, so probably not that big of a change for people
					// since they are already handling this exception when
