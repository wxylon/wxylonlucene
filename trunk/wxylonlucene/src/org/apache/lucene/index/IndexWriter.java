...
public class IndexWriter {
	...
	/**
	 * writeLockTimeout 引用 WRITE_LOCK_TIMEOUT；
	 */
	...
	/**
	 * Default for {@link #getMaxSyncPauseSeconds}. On Windows this defaults to
	 * 10.0 seconds; elsewhere it's 0.
	 */
	...
	volatile SegmentInfos pendingCommit; // set when a commit is pending
											// (after prepareCommit() & before
											// commit())
	...
	private boolean localAutoCommit; // saved autoCommit during local
										// transaction
	private int localFlushedDocCount; // saved docWriter.getFlushedDocCount
	...
	protected synchronized final void ensureOpen()
			throws AlreadyClosedException {
		ensureOpen(true);
	}
	...
	public int getTermIndexInterval() {
		// We pass false because this method is called by SegmentMerger while we
	...
	public IndexWriter(String path, Analyzer a, boolean create,
		...
		init(FSDirectory.getDirectory(path), a, create, true, null, false, mfl
				.getLimit());
	}
	...
	public IndexWriter(String path, Analyzer a, MaxFieldLength mfl)
			throws CorruptIndexException, LockObtainFailedException,
			IOException {
		init(FSDirectory.getDirectory(path), a, true, null, false, mfl
				.getLimit());
	}
	...
	private void init(Directory d, Analyzer a, boolean closeDir,
			...
			int maxFieldLength) throws CorruptIndexException,
			LockObtainFailedException, IOException {
	...
	/**
	 * 此方法描述的是：
	 * @version 创建时间：2009-9-25 上午11:56:21
	 * @param d								指定的存放建立索引文件的索引目录
	 * @param a								Analyzer  分词分析器 
	 * @param create						是否要重新写入索引文件，如果为true，则重写索引文件；如果为false，则追加写入索引文件 
	 * @param closeDir						一个boolean型变量，表示是否关闭索引目录Directory d，与IndexWriter的一个成员变量相关 
	 * @param deletionPolicy				指定删除索引文件使用的策略 
	 * @param autoCommit					建立索引文件后，自动提交。
	 * @param maxFieldLength				表示索引中Field的最大长度。
	...
	private void init(Directory d, Analyzer a, final boolean create,
		...
		directory = d;
		analyzer = a;
		//这里主要是指定infoStream，是一个PrintStream输出流对象 
		...
		try {
			...
			//克隆原来的segment状态信息，并且将信息保存到HashSet。 
			setRollbackSegmentInfos(segmentInfos);
			//创建一个DocumentsWriter对象 
			docWriter = new DocumentsWriter(directory, this);
			//设置DocumentsWriter对象的infoStream信息 
			docWriter.setInfoStream(infoStream);
			//设置DocumentsWriter对象的maxFieldLength信息
	...
	/**
	 * Expert: returns max delay inserted before syncing a commit point. On
	 * Windows, at least, pausing before syncing can increase net indexing
	 * throughput. The delay is variable based on size of the segment's files,
	 * and is only inserted when using ConcurrentMergeScheduler for merges.
	 * 
	 * @deprecated This will be removed in 3.0, when autoCommit=true is removed
	 *             from IndexWriter.
	 */
	...
	private void messageState() {
		message("setInfoStream: dir=" + directory + " autoCommit=" + autoCommit
				...
				+ docWriter.getRAMBufferSizeMB() + " maxBufferedDocs="
	...
	/**
	 * Commits all changes to an index and closes all associated files. Note
	 * that this may be a costly operation, so, try to re-use a single writer
	 * instead of closing and opening a new one. See {@link #commit()} for
	...
	/**
	 * Returns the number of documents currently in this index, not counting
	 * deletions.
	 * 
	 * @deprecated Please use {@link #maxDoc()} (same as this method) or
	 *             {@link #numDocs()} (also takes deletions into account),
	 *             instead.
	 */
	...
	/**
	 * Returns total number of docs in this index, including docs not yet
	 * flushed (still in the RAM buffer), not counting deletions.
	 * 
	 * @see #numDocs
	 */
	...
	/**
	 * Returns total number of docs in this index, including docs not yet
	 * flushed (still in the RAM buffer), and including deletions. <b>NOTE:</b>
	 * buffered deletions are not counted. If you really need these to be
	 ...
	 * 
	 * <p>
	 * This method periodically flushes pending documents to the Directory (see
	 * <a href="#flush">above</a>), and also periodically triggers segment
	 * merges in the index according to the {@link MergePolicy} in use.
	 * </p>
	 * 
	 * <p>
	 ...
	 * index (see {@link #optimize()} for details). The sequence of primitive
	 * merge operations performed is governed by the merge policy.
	 * 
	 * <p>
	...
	/**
	 * Deletes the document(s) matching any of the provided queries. All deletes
	...
	/**
	 * Just like {@link #expungeDeletes()}, except you can specify whether the
	...
	/**
	 * Expunges all deletes from the index. When an index has many document
	 * deletions (or updates to existing documents), it's best to either call
	 * optimize or expungeDeletes to remove all unused data in the index
	 * associated with the deleted documents. To see how many deletions you have
	 * pending in your index, call {@link IndexReader#numDeletedDocs} This saves
	 * disk space and memory usage while searching. expungeDeletes should be
	 * somewhat faster than optimize since it does not insist on reducing the
	 * index to a single segment (though, this depends on the
	 * {@link MergePolicy}; see {@link
	 * MergePolicy#findMergesToExpungeDeletes}.). Note that this call does not
	 * first commit any buffered documents, so you must do so yourself if
	 * necessary. See also {@link #expungeDeletes(boolean)}
	 */
	 ...
	 */
	private synchronized void startTransaction(boolean haveWriteLock)
			throws IOException {
		...
		try {
			...
			assert docWriter.getNumBufferedDeleteTerms() == 0 : "calling startTransaction with buffered delete terms not supported: numBufferedDeleteTerms="
					+ docWriter.getNumBufferedDeleteTerms();
			assert docWriter.getNumDocsInRAM() == 0 : "calling startTransaction with buffered documents not supported: numDocsInRAM="
	 ...
	 * <code>autoCommit=false</code>. This also clears a previous call to
	 * {@link #prepareCommit}.
	 * 
	 ...
	 * 
	 * @deprecated Use {@link #addIndexesNoOptimize} instead, then separately
	 *             call {@link #optimize} afterwards if you need to.
	 * @throws CorruptIndexException
	 *             if the index is corrupt
	 * @throws IOException
	 *             if there is a low-level IO error
				...
				synchronized (this) {
					...
					for (int i = 0; i < dirs.length; i++) {
						...
						for (int j = 0; j < sis.size(); j++) {
							...
							assert !segmentInfos.contains(info) : "dup info dir="
									+ info.dir + " name=" + info.name;
							docCount += info.docCount;
							segmentInfos.add(info); // add each info
						}
					}
	...
	/*
	 * If any of our segments are using a directory != ours then we have to
	 * either copy them over one by one, merge them (if merge policy has chosen
	 * to) or wait until currently running merges (in the background) complete.
	 * We don't return until the SegmentInfos has no more external segments.
	 * Currently this is only used by addIndexesNoOptimize().
	 */
	private void resolveExternalSegments() throws CorruptIndexException,
			IOException {
			...
			synchronized (this) {
				...
				for (int i = 0; i < numSegments; i++) {
					...
					if (info.dir != directory) {
						...
						final MergePolicy.OneMerge newMerge = new MergePolicy.OneMerge(
								segmentInfos.range(i, 1 + i), info
										.getUseCompoundFile());
	 ...
	 * 
	 * <p>
	 * See {@link #addIndexesNoOptimize(Directory[])} for details on
	 * transactional semantics, temporary free space required in the Directory,
		...
		try {
			...
			try {
				...
				try {
					...
					// Notify DocumentsWriter that the flushed count just
					// increased
					docWriter.updateFlushedDocCount(docCount);
	 ...
	 * <p>
	 * Note: while this will force buffered docs to be pushed into the index, it
	 * will not make these docs visible to a reader. Use {@link #commit()}
	 * instead
	 * 
	 * @throws CorruptIndexException
	 *             if the index is corrupt
	 ...
	 * You can only call this when autoCommit is false. This method does all
	 * steps necessary to commit changes since this writer was opened: flushes
	 * pending added and deleted docs, syncs the index files, writes most of
	 * next segments_N file. After calling this you must call either {@link
	 * #commit()} to finish the commit, or {@link #rollback()} to revert the
	 * commit and undo all changes done since the writer was opened.
	 * </p>
	 * 
	...
	private final void prepareCommit(boolean internal)
			throws CorruptIndexException, IOException {
	...
	/**
	 * <p>
	 * Commits all pending updates (added & deleted documents) to the index, and
	 * syncs all referenced index files, such that a reader will see the changes
	 * and the index updates will survive an OS or machine crash or power loss.
	 ...
	 * faster performance. If you have such a device, and it does not have a
	 * battery backup (for example) then on power loss it may still lose data.
	 * Lucene cannot guarantee consistency on such devices.
	 * </p>
	 * 
	 * @see #prepareCommit
	 */
	 ...
	 *            the next segment
	 * @param flushDeletes
	 *            whether pending deletes should also be flushed
	 */
	protected final void flush(boolean triggerMerge, boolean flushDocStores,
		...
		// We can be called during close, when closing==true, so we must pass
		// false to ensureOpen:
		ensureOpen(false);
		...
		try {
			...
			if (infoStream != null) {
				message("  flush: segment=" + docWriter.getSegment()
						...
						+ " docStoreOffset=" + docStoreOffset + " flushDocs="
						+ flushDocs + " flushDeletes=" + flushDeletes
						+ " flushDocStores=" + flushDocStores + " numDocs="
						+ numDocs + " numBufDelTerms="
						+ docWriter.getNumBufferedDeleteTerms());
			...
			if (flushDocs) {
				...
				newSegment = new SegmentInfo(segment, flushedDocCount,
						...
						docStoreSegment, docStoreIsCompoundFile, docWriter
								.hasProx());
			}
				...
				if (segmentInfos.indexOf(info) == -1)
					throw new MergePolicy.MergeException(
							"MergePolicy selected a segment (" + info.name
									+ ") that is not in the index", directory);
				else
					throw new MergePolicy.MergeException(
	 ...
	 * because, although merging will clear all deletes (compacts the
	 * documents), new deletes may have been flushed to the segments since the
	 * merge was started. This method "carries over" such new deletes onto the
	 * newly merged segment, and saves the resulting deletes file (incrementing
	 * the delete generation for merge.info). If no deletes were flushed, no new
		...
		for (int i = 0; i < numSegmentsToMerge; i++) {
			...
			if (previousInfo.hasDeletions()) {
				...
				if (!currentInfo.getDelFileName().equals(
					...
					BitVector currentDeletes = new BitVector(currentInfo.dir,
				...
				if (deletes == null)
					deletes = new BitVector(merge.info.docCount);
				BitVector currentDeletes = new BitVector(directory, currentInfo
						.getDelFileName());
	...
	synchronized private boolean commitMerge(MergePolicy.OneMerge merge,
			SegmentMerger merger, int mergedDocCount) throws IOException {
		...
		docWriter.remapDeletes(segmentInfos, merger.getDocMaps(), merger
				.getDelCounts(), merge, mergedDocCount);
	...
	/**
	 * This is called after merging a segment and before building its CFS.
	 * Return true if the files should be sync'd. If you return false, then the
	 * source segment files that were merged cannot be deleted until the CFS
	 * file is built & sync'd. So, returning false consumes more transient disk
	 * space, but saves performance of not having to sync files which will
	 * shortly be deleted anyway.
	 * 
	 * @deprecated -- this will be removed in 3.0 when autoCommit is hardwired
	 *             to false
	 */
	private synchronized boolean doCommitBeforeMergeCFS(
			MergePolicy.OneMerge merge) throws IOException {
		...
		for (int i = 0; i < size; i++) {
			...
			if (loc != null) {
				final SegmentInfo oldInfo = rollbackSegmentInfos.info(loc
						.intValue());
	...
	final private int mergeMiddle(MergePolicy.OneMerge merge)
		...
		try {
			...
			for (int i = 0; i < numSegments; i++) {
				...
				IndexReader reader = SegmentReader.get(true, si,
	...
	private final synchronized boolean applyDeletes()
		...
		try {
			...
			if (!success) {
				...
				for (int i = 0; i < size; i++) {
					final String newDelFileName = segmentInfos.info(i)
							.getDelFileName();
	...
	/**
	 * Pauses before syncing. On Windows, at least, it's best (performance-wise)
	 * to pause in order to let OS flush writes to disk on its own, before
	 * forcing a sync.
	 * 
	 * @deprecated -- this will be removed in 3.0 when autoCommit is hardwired
	 *             to false
		...
		try {
			...
			synchronized (this) {
				...
				try {
					...
					if (infoStream != null)
						message("startCommit index=" + segString(segmentInfos)
			...
			try {
					...
					for (int i = 0; i < toSync.size(); i++) {
						...
						for (int j = 0; j < files.size(); j++) {
							...
							if (startSync(fileName, pending)) {
								boolean success = false;
								try {
									// Because we incRef'd this commit point,
									// above,
									// the file had better exist:
									assert directory.fileExists(fileName) : "file '"
											+ fileName
											+ "' does not exist dir="
											+ directory;
				...
				synchronized (this) {
					...
					if (myChangeCount > lastCommitChangeCount
							&& (pendingCommit == null || myChangeCount > pendingCommitChangeCount)) {
						...
						if (segmentInfos.getGeneration() > toSync
								.getGeneration())
	 ...
	 * {@link #setMaxFieldLength(int)} overrides the value set by the
	 * constructor.
	 */
	public static final class MaxFieldLength {
		...
		/**
		 * Public constructor to allow users to specify the maximum field size
		 * limit.
		 * 
		 * @param limit
		...
		public static final MaxFieldLength LIMITED = new MaxFieldLength(
				"LIMITED", DEFAULT_MAX_FIELD_LENGTH);
	}
